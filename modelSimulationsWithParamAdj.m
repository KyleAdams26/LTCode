function modelSimulationsWithParamAdj
    ...
    [T1, Y1, T2, Y2] = solveWithModifiedParam('aCL', param, tspan); L1 = Y1(:,6); L2 = Y2(:,6);
    [T3, Y3, T4, Y4] = solveWithModifiedParam('dL', param, tspan); L3 = Y3(:,6); L4 = Y4(:,6);
    [T5, Y5, T6, Y6] = solveWithModifiedParam('bCL', param, tspan); L5 = Y5(:,6); L6 = Y6(:,6);
    [T7, Y7, T8, Y8] = solveWithModifiedParam('KC', param, tspan); L7 = Y7(:,6); L8 = Y8(:,6);
    [T9, Y9, T10, Y10] = solveWithModifiedParam('gC', param, tspan); L9 = Y9(:,6); L10 = Y10(:,6);
    [T11, Y11, T12, Y12] = solveWithModifiedParam('aIC', param, tspan); L11 = Y11(:,6); L12 = Y12(:,6);
    ...
    addTile('gC', T9, L9, T10, L10, L_color, 30);
    addTile('aIC', T11, L11, T12, L12, L_color, 30);
    hold off;
    ...
    % the QOI
        ...
        [T_favor, Y_favor] = ode45(@(t, y) odefun(t, y, p_favor), tspan, IC, options); L_favor = Y_favor(:,6);
        [T_against, Y_against] = ode45(@(t, y) odefun(t, y, p_against), tspan, IC, options); L_against = Y_against(:,6);
        %plot figure
        figure;
        hold on;
